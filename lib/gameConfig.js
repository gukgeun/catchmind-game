...
export function pickNextDrawerUid(playersObj, currentDrawerUid, excludeUid) {
  const players = sortPlayersByOrder(playersObj).filter((p) => p.online && p.uid !== excludeUid);
...
export function computeWinners(playersObj, excludeUid) {
  const players = sortPlayersByOrder(playersObj).filter((p) => p.uid !== excludeUid);
